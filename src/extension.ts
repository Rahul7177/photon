...
import { installUnifiedRuntime } from "./core/agent/unifiedEngine";

export function activate(context: vscode.ExtensionContext): void {
  // Install the v2 execution policy before the controller creates an AgentEngine.
  // The shim keeps the public controller contract stable while making local and
  // cloud turns share the same stateful planner/executor.
  ...
  context.subscriptions.push(vscode.window.registerWebviewViewProvider(ChatViewProvider.viewType, provider, { webviewOptions: { retainContextWhenHidden: true } }));
