...
export async function runBench(client:LLMProvider,model:string,opts:BenchOptions):Promise<BenchResult>{
  ...
  for(let i=0;i<TOOLCALL_ATTEMPTS;i++){
    ...
    const good=parsePhotonBlocks(out.text,[PROBE_TOOL]).calls.some(c=>c.name==="read_file"&&!c.errors.length&&c.args.path==="src/index.ts");
    if(good)toolPasses++;
  }
  ...
  const context=await generate(client,model,[
    {role:"user",content:"Remember this identifier for the next message: PHOTON_CTX_7429."},
    {role:"assistant",content:"Remembered."},
    {role:"user",content:"What identifier did I ask you to remember? Reply with the identifier only."},
  ],opts.signal);
  ...
  const profile:ModelCapabilityProfile={
    reasoning:tasks.find(t=>t.id==="reasoning")?.passed?0.9:0.35,
    coding:tasks.find(t=>t.id==="edit")?.passed?0.88:0.45,
    toolCalling:toolReliability,
    schemaAdherence:tasks.find(t=>t.id==="schema")?.passed?Math.min(1,toolReliability+0.05):Math.max(0,toolReliability-0.15),
    contextRetention:contextPass?0.9:0.35,
    editFidelity:editPass?0.88:0.4,
    recovery:recoveryPass?0.82:0.35,
    verification:verificationPass?0.85:0.4,
    speed:1,
  };
