...
export class GeminiProvider implements LLMProvider {
  ...
  async fetchLiveModels(): Promise<ModelInfo[]> {
    ...
    const json = await res.json() as { models?: { name: string; displayName?: string; supportedGenerationMethods?: string[]; inputTokenLimit?: number; outputTokenLimit?: number }[] };
    const out: ModelInfo[] = [];
    for (const m of json.models ?? []) {
      if (!m.supportedGenerationMethods?.includes("generateContent")) continue;
      const rawId = m.name.replace(/^models\//, ""); const caps = geminiCaps(rawId);
      out.push({ name: `gemini:${rawId}`, provider: "gemini", contextLength: m.inputTokenLimit, toolTrained: true, vision: caps.includes("vision"), audio: caps.includes("audio"), video: caps.includes("video"), thinking: caps.includes("thinking"), capabilities: caps, tier: "large" });
    }
    return out;
  }
