...
/** True when a request should be backed by fresh external information rather than model memory. */
export function isFreshInfoRequest(prompt:string):boolean{
  const text=prompt??"";
  const current=/\\b(today|tonight|now|right now|currently|current|latest|recent|live|real[- ]?time|as of|this morning|this afternoon|this evening)\\b/i.test(text);
  const external=/\\b(price|prices|stock|stocks|share|shares|nifty|sensex|market|weather|forecast|news|exchange rate|forex|currency|bitcoin|crypto|score|scores|standings|schedule|release|version|updates?|time|date)\\b/i.test(text);
  const explicitWeb=/\\bweb search\\b|\\b(search|check|look up|browse|find).{0,40}\\b(web|internet|online)\\b/i.test(text);
  return explicitWeb||(current&&external);
}
...
export function capabilityForModel(model:ModelInfo,bench?:BenchResult):ModelCapabilityProfile{
  ...
  const baseMap:Record<string,number>={tiny:.38,small:.55,medium:.72,large:.88};
  const base=baseMap[tier]??.55;
  ...
  if(bench){
    p.speed=Math.max(.05,Math.min(1,Math.log1p(Math.max(0,bench.tokensPerSec))/Math.log1p(100)));
    if(bench.capabilityProfile)return clampProfile({...p,...bench.capabilityProfile});
    p.toolCalling=bench.toolCallReliability;
    p.schemaAdherence=bench.toolCallReliability;
    p.reasoning=taskPassRate(bench,"reasoning",p.reasoning);
    p.recovery=taskPassRate(bench,"recovery",p.recovery);
    p.editFidelity=taskPassRate(bench,"edit",p.editFidelity);
    p.verification=taskPassRate(bench,"verification",p.verification);
    p.contextRetention=taskPassRate(bench,"context",p.contextRetention);
    p.coding=taskPassRate(bench,"edit",p.coding);
  }
...
export function rankTools(specs:ToolSpec[],task:TaskAnalysis,phase:"orient"|"edit"|"verify"|"final",previousFailures=new Map<string,number>()):ToolSpec[]{
  const verification=new Set(task.verification);
  return[...specs].sort((a,b)=>scoreTool(b,task,phase,verification,previousFailures)-scoreTool(a,task,phase,verification,previousFailures));
}
function scoreTool(tool:ToolSpec,task:TaskAnalysis,phase:string,verification:Set<VerificationKind>,failures:Map<string,number>):number{
  ...
  if(phase==="orient"&&(tags.has("read")||tags.has("search")||tags.has("navigate")||tags.has("web")))score+=35;
  ...
  if((failures.get(tool.name)??0)>=2)score+=5;
  return score;
}
...
export function buildExecutionPolicy(task:TaskAnalysis,capability:ModelCapabilityProfile,maxSteps:number,maxTools:number):AdaptivePlan["executionPolicy"]{
  const concurrency=capability.toolCalling>=.78&&capability.recovery>=.60?4:capability.toolCalling>=.60?2:1;
  const reasoningBudget=task.reasoning==="high"?Math.round(3072+Math.max(0,capability.reasoning-.7)*3400):task.reasoning==="medium"?Math.round(768+Math.max(0,capability.reasoning-.65)*1200):0;
  const visibleBudget=task.reasoning==="high"?4096:task.reasoning==="medium"?2048:1024;
  const generationBudgetTokens=Math.max(512,Math.min(8192,visibleBudget+reasoningBudget));
  return{maxConcurrent:task.risk==="destructive"?1:Math.min(concurrency,4),allowParallelReads:task.risk!=="destructive",serializeMutations:true,generationBudgetTokens,stepBudget:Math.max(8,Math.min(200,Math.max(maxSteps,task.estimatedSteps*3,maxTools*2)))};
}
